...
| **References** | Tracks referee requests, institutions, requested/deadline dates, reminders, and received/declined status. The app never sends requests. |
| **Interview preparation** | Generates opportunity-grounded practice questions and stores your own answers and notes per application. |
| **Form memory** | Approved, non-sensitive answers can be stored per university site and reused by the extension; sensitive fields remain manual. |
| **Exports** | Generated SOPs, cover letters and proposals can be downloaded as TXT, DOCX or PDF. |
